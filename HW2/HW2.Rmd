...
```{r, eval = FALSE}
#Unweighted filter
influential_vars <- summary.gbm(gbm0,main="RELATIVE INFLUENCE",plotit=FALSE)
top5_unweighted <- gsub("`","",influential_vars$var[1:5])

#Weighted filter
influential_vars <- summary.gbm(gbm1,main="RELATIVE INFLUENCE",plotit=FALSE)
...
Partial dependence plots show how a model's output affect is when varying a subset of variables (accounting for the other complementing subset of variables). For this problem, we will at single variable partial dependence plots. Looking at the partial dependence, we can understand what the effect of increasing or decreasing the single variable in questions's value has on the model's output (accounting for all other variables). This gives us a better understanding of the behavior of otherwise "black box" models.

\textbf{Unweighted model:} Looking at the variable with the most relative influence, \textit{"!"}, we can see that as we increase the frequency of \textit{"!"} as a proportion of an email's length, it increases the model's probability that this is spam. This means that emails with a high frequency of \textit{"!"} relative to their length are indicative of spam since spam is the positve class and nonspam is the zero class.
...
The other top influential variables have similar dependence plots: the variable influences the model's output to a particular class as its value is increased while taking into account the effects of the other variables.

\textbf{Weighted model:} The partial dependence plots of the weighted model are harder to interpret. We have used the weights to force our model to learn

```{r, eval = FALSE}
plot(gbm0,match("!", rflabs),  nTreesForPrediction0, type="response",height=2)
plot(gbm0,match("hp", rflabs), nTreesForPrediction0, type="response",height=2)

plot(gbm1,match("CAPAVE", rflabs),   nTreesForPrediction1, type="link",height=2)
plot(gbm1,match("business", rflabs), nTreesForPrediction1, type="link",height=2)
```
