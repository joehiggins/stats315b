...
\frac{\partial L}{\partial\sigma_k} &= \frac{a_k}{\sigma^3_k}\sum_{i=1}^N(\hat{F}(\mathbf{x}_i)-y_i)B(\mathbf{x}_i|\mathbf{\mu}_k,\sigma_k)(\mathbf{x}_i -\mu_k)^T(\mathbf{x}_i - \mu_k) \\
 \frac{\partial L}{\partial a_k} &= \sum_{i=1}^N(\hat{F}(\mathbf{x}_i)-y_i)B(\mathbf{x}|\mu_k,\sigma_k) \\
\end{aligned}
$$
...
$$
\begin{aligned}
(2\mathbf{x} + \mathbf{v}_j)^T\Sigma_m\mathbf{v}_j &= \sum_{j=1}^p\lambda_j(2\mathbf{x} + \mathbf{v}_j)^T\mathbf{v}_j\mathbf{v}_j^T\mathbf{v}_j \\
